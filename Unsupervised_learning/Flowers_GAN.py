...
import Dataset_Reader.read_FlowersDataset as flowers

FLAGS = tf.flags.FLAGS
tf.flags.DEFINE_integer("batch_size", "64", "batch size for training")
tf.flags.DEFINE_string("logs_dir", "logs/Flowers_GAN_logs/", "path to logs directory")
tf.flags.DEFINE_string("data_dir", "Data_zoo/flowers_data/", "path to dataset")
...
def generator(z, train_mode):
    with tf.variable_scope("generator") as scope:
        W_0 = utils.weight_variable([FLAGS.z_dim, 64 * 16 * IMAGE_SIZE / 32 * IMAGE_SIZE / 32], name="W_0")
        b_0 = utils.bias_variable([64 * 16 * IMAGE_SIZE / 32 * IMAGE_SIZE / 32], name="b_0")
        z_0 = tf.matmul(z, W_0) + b_0
        h_0 = tf.reshape(z_0, [-1, IMAGE_SIZE / 32, IMAGE_SIZE / 32, 64 * 16])
        ...
        W_1 = utils.weight_variable_xavier_initialized([5, 5, 64 * 8, 64 * 16], name="W_1")
        b_1 = utils.bias_variable([64 * 8], name="b_1")
        deconv_shape = tf.pack([tf.shape(h_relu0)[0], IMAGE_SIZE / 16, IMAGE_SIZE / 16, 64 * 8])
        ...
        W_2 = utils.weight_variable_xavier_initialized([5, 5, 64 * 4, 64 * 8], name="W_2")
        b_2 = utils.bias_variable([64 * 4], name="b_2")
        deconv_shape = tf.pack([tf.shape(h_relu1)[0], IMAGE_SIZE / 8, IMAGE_SIZE / 8, 64 * 4])
        ...
        W_3 = utils.weight_variable_xavier_initialized([5, 5, 64 * 2, 64 * 4], name="W_3")
        b_3 = utils.bias_variable([64 * 2], name="b_3")
        deconv_shape = tf.pack([tf.shape(h_relu2)[0], IMAGE_SIZE / 4, IMAGE_SIZE / 4, 64 * 2])
        h_conv_t3 = utils.conv2d_transpose_strided(h_relu2, W_3, b_3, output_shape=deconv_shape)
        h_relu3 = tf.nn.relu(h_conv_t3, name='relu3')
        utils.add_activation_summary(h_relu3)

        W_4 = utils.weight_variable_xavier_initialized([5, 5, 64 * 1, 64 * 2], name="W_4")
        b_4 = utils.bias_variable([64 * 1], name="b_4")
        deconv_shape = tf.pack([tf.shape(h_relu3)[0], IMAGE_SIZE / 2, IMAGE_SIZE / 2, 64 * 1])
        h_conv_t4 = utils.conv2d_transpose_strided(h_relu3, W_4, b_4, output_shape=deconv_shape)
        h_relu4 = tf.nn.relu(h_conv_t4, name='relu4')
        utils.add_activation_summary(h_relu4)

        W_5 = utils.weight_variable_xavier_initialized([5, 5, 3, 64 * 1], name="W_5")
...
def main(argv=None):
    train_images, valid_images, test_images = flowers.read_flowers_dataset(FLAGS.data_dir)
